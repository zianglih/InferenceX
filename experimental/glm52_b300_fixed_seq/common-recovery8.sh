...
finish_glm52_case() {
    ...
    python3 "$EXPERIMENT_DIR/artifacts.py" finish "$work_status" || artifact_status=$?
    if [[ "$work_status" -eq 0 && "$artifact_status" -eq 0 ]]; then
        python3 "$CAMPAIGN_HELPER_ROOT/cli.py" join --lock "$R8_EXECUTION_LOCK" --lock-sha "$R8_EXECUTION_LOCK_SHA256" --case "$SCENARIO/w4a16-megamoe/tp${TP}_conc${CONC}" || artifact_status=$?
    fi
