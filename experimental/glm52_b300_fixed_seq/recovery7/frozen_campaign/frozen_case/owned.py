...
The same-birth group proof follows the accepted D6 cleanup rule. This adapter
uses kernel starttime rather than psutil timestamps and tracks only child trees.
"""
import os
...
def same(now, birth):
    return now is not None and now['pid'] == birth['pid'] and now['starttime'] == birth['starttime']


class Owned:
    def __init__(self, proc, label, persist, read=proc_info):
    ...
    def refresh(self):
        pending = list(self.known.values())
        seen = set()
        while pending:
            prior = pending.pop()
            ...
            for tid in tids:
                ...
                for child_pid in map(int, raw.split()):
                    ...
                    if child['ppid'] != prior['pid']:
                        continue
                    if child_pid in self.known and not same(child, self.known[child_pid]):
                        raise ValueError('Known child PID reused')
                    self.known[child_pid] = child
                    pending.append(child)
                    if len(self.known) > MAX_PROCESSES:
                        raise ValueError('Owned descendant count exceeds bound')
        self.persist(self)
    ...
    def bind_rank(self, owner):
        ...
        if known is None or not same(current, known) or not same(current, owner) or current['state'] == 'Z':
    ...
    def signal_owned(self, sig):
        ...
        if any(x['pgid'] == self.proc.pid and x['sid'] == self.proc.pid for x in living):
            try:
        ...
        if self.birth is None:
            ...
            for sig, delay in ((signal.SIGTERM, term), (signal.SIGKILL, kill)):
                ...
                try:
                    ...
                    pass
        remaining = self.live()
        return {'label': self.label, 'leader_pid': self.proc.pid, 'birth': self.birth,
                'returncode': self.proc.poll(), 'remaining': remaining, 'errors': self.errors,
                'cleanup_pending': bool(remaining or self.errors or self.proc.poll() is None)}


class Registry:
    def __init__(self, persist, popen=subprocess.Popen, read=proc_info):
        self.persist, self.popen, self.read, self.children = persist, popen, read, []

    def spawn(self, argv, *, env, output, label, new_session=False):
        proc = self.popen(argv, env=env, stdout=output, stderr=subprocess.STDOUT,
                          start_new_session=new_session)
        child = Owned(proc, label, self.persist, self.read)
    ...
    def cleanup(self):
        ...
        for child in reversed(self.children):
            try:
                ...
                receipts.append({'label': child.label, 'leader_pid': child.proc.pid,
                                 'cleanup_pending': True, 'error': repr(error)})
        return receipts
