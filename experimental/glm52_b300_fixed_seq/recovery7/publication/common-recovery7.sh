...
finish_glm52_case() {
    ...
    if [[ "$work_status" -eq 0 && "$artifact_status" -eq 0 ]]; then
        python3 "$CAMPAIGN_HELPER_ROOT/cli.py" join --lock "$R7_EXECUTION_LOCK" --lock-sha "$R7_EXECUTION_LOCK_SHA256" --case "$SCENARIO/w4a16-megamoe/tp${TP}_conc${CONC}" || artifact_status=$?
    fi
