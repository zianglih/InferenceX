#!/usr/bin/env python3
"""Clean-node, real-weight recovery4 preparation. Default plan is local-only.
...
ROOT = Path("/data/home/ziangli/inferencex-glm52-megamoe-autotune-20260921-recovery4")
BOOTSTRAP = Path(
    "/data/home/ziangli/inferencex-glm52-megamoe-bootstrap-20260921-recovery4"
)
...
def load(name):
    ...
    spec = importlib.util.spec_from_file_location("_recovery4_" + path.stem, path)
...
def checked_identity(receipt, hostname):
    require(
        ...
        "Wrong recovery4 node/image/queue identity",
    )
    require(
...
def published_helpers(recipe, relative):
    ...
    expected = {
        ...
        for name in ("bootstrap_megamoe_recovery4.py", "continue_megamoe_recovery4.py")
...
def plan():
    ...
    return {
        "identity": IDENTITY,
        "physical_host": "requires actual caller receipt",
        "task_root": str(ROOT),
        "bootstrap_root": str(BOOTSTRAP),
        "run_id": RUN,
        "sglang_commit": b.SG,
