...
def setup_guard(lock, lock_path):
    ...
    core.need(
        setup["status"] == "PREPARED_R8_NO_BENCHMARK" and setup["error"] is None,
    ...
    layout = core.module("layout.py", "_r8_layout_check")
...
def archive(lock, code):
    ...
    guard = core.module("guard.py", "_r8_archive_guard")
    guard.ROOT=core.OLD
    idle = guard.idle(lock["runtime_guard"])
    core.write(core.ROOT / "launcher/archive-idle.json", idle)
    old = core.module("archive.py", "_r8_archive_private")
...
def worker(lock, lock_path):
    ...
    try:
        ...
        env["R8_EXECUTION_LOCK_SHA256"] = core.describe(lock_path)["sha256"]
        env["R8_CAMPAIGN_WORKER_PID"] = str(os.getpid())
        wrapper = (
            core.recipe_files(core.ROOT / "sources/inferencex")
            / "run_megamoe_recovery8.sh"
        )
