"""R8 fixed-resource preparation primitives; no work occurs on import."""
...
ROOT = Path("/data/home/ziangli/inferencex-glm52-megamoe-autotune-20260921-recovery8")
...
def load_lock(path, expected):
    ...
    need(
        lock.get("status") == "BOUND_FOR_REVIEWED_R8_EXECUTION",
        "Activation remains unbound",
    )
    need(
        lock.get("root") == str(ROOT) and lock.get("run_id") == RUN, "Wrong R8 root/run"
    )
    node=lock.get('node',{})
...
def host_guard(lock):
    ...
    guard = module("guard.py", "_r8_guard")
    guard.ROOT=OLD
    result = guard.guards(lock["runtime_guard"])
    checkpoint = module("checkpoint.py", "_r8_checkpoint").inspect_model()
    expected = read(
...
def retain_sglang_parent(seed, destination):
    """Copy the exact verified parent object into this independent shallow clone."""
    seed, destination = safe(seed), safe(destination)
    need(seed.is_absolute() and destination.is_absolute() and seed != destination, 'Exact local parent source required')
    # The seed is verified before clone; no remote URL or ref name is accepted here.
    need(cmd(['git', '-C', str(seed), 'rev-parse', BOOTSTRAP_SG + '^{commit}']) == BOOTSTRAP_SG, 'Verified seed parent missing')
    cmd(['git', '-c', 'protocol.allow=never', '-c', 'protocol.file.allow=always',
         '-C', str(destination), 'fetch', '--no-tags', '--no-write-fetch-head',
         '--depth=1', str(seed), BOOTSTRAP_SG],
        env={'GIT_CONFIG_NOSYSTEM': '1', 'GIT_CONFIG_GLOBAL': '/dev/null'}, timeout=120)
    need(cmd(['git', '-C', str(destination), 'rev-parse', BOOTSTRAP_SG + '^{commit}']) == BOOTSTRAP_SG, 'Copied parent object differs')
...
def published_equality(recipe):
    ...
    installed = recipe_files(recipe) / "recovery8"
    ...
    for name in ("run_megamoe_recovery8.sh", "common-recovery8.sh"):
        need(
...
def prepare(lock, lock_path):
    ...
    need(not ROOT.exists() and not ROOT.is_symlink(), "R8 root must be exclusive")
    ...
    try:
        ...
        terminal.update(
            status="PREPARED_R8_NO_BENCHMARK",
            source_pins={
                "sglang": SG,
                "flashinfer": FI,
...
def clean_parent(lock):
    ...
    env = dict(os.environ)
    for key in list(env):
        if key.startswith(("NVSHMEM_", "MEGAMOE_R8_NATIVE_")) or key in (
            "LD_PRELOAD",
            "LD_AUDIT",
