...
def wrap(module, lock, output, capture_fn=capture, identity_fn=identity):
    ...
    require(not getattr(original,'_r8_native_proof',False), 'Duplicate natural hook')
    ...
    def observed(*args,**kwargs):
        ...
        return result
    observed._r8_native_proof=True
...
def install():
    name=os.environ.get('MEGAMOE_R8_NATIVE_PROOF_LOCK')
    if not name: return
    p=Path(name)
    require(p.is_absolute() and not any(x.is_symlink() for x in (p,*p.parents)), 'Unsafe hook lock')
    b=p.read_bytes();require(len(b)<=64*1024 and sha(b)==os.environ.get('MEGAMOE_R8_NATIVE_PROOF_SHA256'), 'Hook lock hash differs')
    lock=json.loads(b)
    require(lock.get('execution_status')=='REVIEWED_SUCCESSOR_REQUIRED', 'Prototype never installs against candidate/null bindings')
    require(lock.get('run_id')=='c2-w4a16-megamoe-autotune-20260921-recovery8', 'Wrong run identity')
    validate_lock(lock)
