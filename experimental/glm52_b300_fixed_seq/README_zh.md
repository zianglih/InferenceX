...
- **W4A4 TRT-LLM：**C2 测量已于 2026-09-19 完成，全部 16 个点和 10,240/10,240 个
  正式请求通过。参见[结果、图表与运行时限制](results/c2-w4a4-20260919/README_zh.md)。
  保留原来的 TP4/DP1/EP1 并发 sweep，以及 TP8/DP1/EP1、并发 4 的点。
- **优化版 W4A16 MegaMoE：**已准备固定 FlashInfer PR #5019 源码与原生 TRT-LLM
  BF16 MTP draft。新一轮 16 点测量待完成；配置就绪不代表性能结果。先 source 基础
...
已有的 FlashInfer。MegaMoE 另通过 [`config-megamoe.env`](config-megamoe.env) 固定
...
| Draft MoE | W4A4：原生继承设置；MegaMoE：`flashinfer_trtllm`、A2A `none`；两者均不显式覆盖 draft 量化 |
...
W4A4 保留原生 draft 设置。优化版 MegaMoE 显式设置
...
替代此前 Triton/unquant draft 配置。与 baseline 一样，序列化的 draft 量化设置可以
继承 `modelopt_fp4`，但 GLM NextN decoder 以 `quant_config=None` 构造原生 BF16
MoE，继承标签不会把其权重变成 FP4。Draft MoE backend 现在与 baseline 相同，但
执行拓扑不同：MegaMoE 方案是 DP=TP=EP 并启用 DP attention，baseline 是 DP1/EP1。
两者不能描述为完全相同的 draft 执行路径。
