#!/usr/bin/env python3
"""Launch only the fourteen real-weight recovery5 points after reviewed bootstrap.
...
def bootstrap():
    path = Path(__file__).with_name("bootstrap_megamoe_recovery5.py")
    spec = importlib.util.spec_from_file_location("_recovery5_bootstrap", path)
...
def verify(c):
    ...
    c.require(
        Path(__file__).resolve() == evidence / "helpers/continue_megamoe_recovery5.py"
        and Path(c.__file__).resolve()
        == evidence / "helpers/bootstrap_megamoe_recovery5.py",
...
def launch(c):
    ...
    script = c.ROOT / "environment/helpers/continue_megamoe_recovery5.py"
