# MegaMoE recovery8: SGLang workaround with stock NVSHMEM

**English** | [中文](./RECOVERY8_zh.md)
...
The portable entry point is `recovery8/cli.py`; it performs no HAI operations. Its shipped activation template is unbound and rejects execution. The campaign root/run are `/data/home/ziangli/inferencex-glm52-megamoe-autotune-20260921-recovery8` / `c2-w4a16-megamoe-autotune-20260921-recovery8`. Bind actual node identity, GPU UUIDs, current bootstrap/source/provider receipts and predecessor process births from the new resource. Historical records do not establish current liveness.
...
## Source checkout continuation

Recovery7 stopped before any benchmark because a new shallow SGLang clone omitted the parent object used by the exact source-diff check. Recovery8 preserves that failed root and explicitly fetches the exact parent from the already verified local seed before running the unchanged parent, diff and blob checks. It uses a new run/root identity; model, source pins, native configuration, clients and all fourteen remaining coordinates are unchanged. No completed point is rerun.
