#!/usr/bin/env python3
"""Render the C4-C64 subset of the published GLM-5.2 R2 table coordinates and four frontiers.
...
def plot(rows, out):
    ...
    ax.set(xlabel="Interactivity = 1,000 / saved median TPOT (tokens/s/user)", ylabel="Whole-interval output throughput (tokens/s/GPU)", xlim=(80, 207), ylim=(0, 1500))
    ax.grid(alpha=.20)
    ax.spines[["top", "right"]].set_visible(False)
    ax.legend(loc="upper right", frameon=True, framealpha=.96, fontsize=11)
    fig.suptitle("GLM-5.2 MegaMoE | W4A4 versus W4A16 | B300 | C4-C64", y=.965, fontsize=20, weight="bold")
    fig.text(.5, .915, "Nominal 1,024 input / 8,192 output | ratio 0.8 | 20 displayed points | 4,960 measured requests", ha="center", fontsize=12)
    fig.text(.5, .115, "Each line connects nondominated points within its own precision/topology group; concurrency 4, 8, 16, 32, 64.", ha="center", fontsize=10)
...
def main():
    ...
    all_rows, _ = load_selected(root)
    rows = [r for r in all_rows if r['concurrency'] <= 64]
    assert len(rows) == 20 and sum(r['completed'] for r in rows) == 4960
    out.mkdir(parents=True, exist_ok=False)
    write_json(out/'selected-metrics.json', rows)
    plot(rows, out)
    write_json(out/'RENDER_INPUTS.json', {
        'published_manifest_sha256': MANIFEST_SHA,
        'filter': 'concurrency <= 64',
        'displayed_points': 20, 'displayed_measured_requests': 4960,
        'colors': {'w4a4': '#009E73', 'w4a16': '#E69F00'},
        'ep_styles': {'4': 'solid/circle', '8': 'dashed/triangle'},
        'frontiers': {f'{p}-tp{tp}': [r['case_id'] for r in pareto_frontier([r for r in rows if (r['precision'], r['tp']) == (p, tp)])] for p,tp in GROUPS},
        'scope': 'Filtered view of accepted R2 measurements. The full C128 figure and raw results are unchanged.'})
    write_json(out/'FILES.json', {f.name: desc(f) for f in sorted(out.iterdir()) if f.is_file()})
    print(json.dumps({'status':'PASS','points':20,'requests':4960,'output':str(out)}))
